...
const defaultValues = {
  ...
  fromAddress: 'Mumbai, Maharashtra, India',
...
const baseFields = [
  ...
  { name: 'fromName', label: 'From (Agency Name)', type: 'text', required: true },
  { name: 'fromAddress', label: 'From Address', type: 'text' },
  { name: 'fromEmail', label: 'From Email', type: 'email' },
  { name: 'fromPhone', label: 'From Phone', type: 'tel' },
  { name: 'toName', label: 'To (Client Name)', type: 'text', required: true },
  { name: 'toCompany', label: 'To Company', type: 'text' },
  { name: 'toAddress', label: 'To Address', type: 'text' },
  { name: 'toEmail', label: 'To Email', type: 'email' },
  { name: 'taxRate', label: 'Tax Rate (%)', type: 'number', min: 0, max: 100 },
  { name: 'discountPercent', label: 'Discount (%)', type: 'number', min: 0, max: 100 },
  { name: 'notes', label: 'Notes', type: 'textarea', rows: 2 },
  { name: 'bankName', label: 'Bank Name', type: 'text' },
  { name: 'accountName', label: 'Account Name', type: 'text' },
  { name: 'accountNumber', label: 'Account Number', type: 'text' },
  { name: 'ifscCode', label: 'IFSC/Swift Code', type: 'text' },
...
const InvoiceTool = () => {
  ...
  const generatePDF = () => {
    ...
    doc.text(formData.invoiceNumber, 15, 38);
    
    const fromX = pageWidth - 80;
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(17, 17, 17);
    doc.text('FROM', fromX, 15);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(100, 100, 100);
    doc.text(formData.fromName || 'Agency', fromX, 22);
    if (formData.fromAddress) doc.text(formData.fromAddress, fromX, 28);
    if (formData.fromEmail) doc.text(formData.fromEmail, fromX, 34);
    if (formData.fromPhone) doc.text(formData.fromPhone, fromX, 40);
    
    doc.text('BILL TO', 15, 58);
    doc.setFont('helvetica', 'bold');
    doc.text(formData.toName || 'Client', 15, 65);
    doc.setFont('helvetica', 'normal');
    if (formData.toCompany) doc.text(formData.toCompany, 15, 71);
    if (formData.toAddress) doc.text(formData.toAddress, 15, 77);
    if (formData.toEmail) doc.text(formData.toEmail, 15, 83);
    ...
    if (formData.bankName || formData.upiId) {
      ...
      if (formData.bankName) {
        doc.text(`Bank: ${formData.bankName}`, 15, yPos);
        yPos += 6;
      }
      if (formData.accountName) {
        doc.text(`Account Name: ${formData.accountName}`, 15, yPos);
        yPos += 6;
      }
      if (formData.accountNumber) {
        doc.text(`Account No: ${formData.accountNumber}`, 15, yPos);
        yPos += 6;
      }
      if (formData.ifscCode) {
        doc.text(`IFSC: ${formData.ifscCode}`, 15, yPos);
        yPos += 6;
      }
      if (formData.upiId) {
        doc.text(`UPI: ${formData.upiId}`, 15, yPos);
        yPos += 6;
      }
    }
    ...
    <div className="bg-white rounded-lg p-6 text-sm font-sans">
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="text-right">
          <p className="text-gray-600">{formData.fromName}</p>
          <p className="text-sm text-gray-400">{formData.invoiceDate}</p>
        </div>
      </div>
      
      <div className="mb-6">
        <p className="text-xs text-gray-400 mb-1">Bill To:</p>
        <p className="font-medium">{formData.toName || '[Client]'}</p>
        <p className="text-sm text-gray-500">{formData.toCompany}</p>
      </div>
      ...
      <div className="text-right space-y-1 text-sm">
        ...
        {calculateDiscount() > 0 && (
          <p>Discount: <span className="text-gray-600">-${calculateDiscount().toLocaleString()}</span></p>
        )}
        {calculateTax() > 0 && (
          <p>Tax ({formData.taxRate}%): <span className="text-gray-600">${calculateTax().toLocaleString()}</span></p>
        )}
        <p className="font-bold text-lg">Total: <span className="text-red-500">${calculateTotal().toLocaleString()}</span></p>
      </div>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          <div>
            ...
            <div className="bg-surface p-6 rounded-2xl border border-primary/5 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-4">
                  ...
                  <input
                    ...
                    onChange={(e) => handleChange('toCompany', e.target.value)}
                    className="w-full p-3 bg-bg-base border border-primary/10 rounded-lg text-sm"
                  />
                  <input
                    type="email"
                ...
                {formData.items.map((item, index) => (
                  <div key={index} className="flex gap-3 items-start">
                    <input
                      ...
                      className="flex-1 p-3 bg-bg-base border border-primary/10 rounded-lg text-sm"
                    />
                    <input
                      type="number"
                      placeholder="Qty"
                      ...
                      className="w-20 p-3 bg-bg-base border border-primary/10 rounded-lg text-sm text-center"
                    />
                    <input
                      type="number"
                      placeholder="Rate"
                      ...
                      className="w-28 p-3 bg-bg-base border border-primary/10 rounded-lg text-sm text-right"
                    />
                    <button
                ...
                <div className="w-64 space-y-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-16 p-1 bg-bg-base border border-primary/10 rounded text-center text-xs"
                    />
                    <span>%</span>
            ...
            <div className="mt-6 p-6 bg-surface rounded-2xl border border-primary/5">
              <h3 className="font-medium mb-4">Payment Details (Optional)</h3>
              <div className="grid grid-cols-2 gap-4">
                <input
                  ...
                  className="p-3 bg-bg-base border border-primary/10 rounded-lg text-sm"
                />
                <input
                  type="text"
                  placeholder="UPI ID"
              ...
              <textarea
                ...
                className="mt-4 w-full p-3 bg-bg-base border border-primary/10 rounded-lg text-sm resize-none h-20"
              />
            </div>

            <button
              onClick={generatePDF}
              className="mt-6 w-full py-4 px-6 bg-accent text-surface font-medium rounded-xl flex items-center justify-center gap-2 hover:bg-accent/90 transition-colors"
            >
          ...
          <div className="hidden lg:block sticky top-24">
            ...
            <div className="bg-bg-tert rounded-2xl p-4 md:p-8 min-h-[600px] pdf-preview overflow-auto max-h-[800px]">
              {previewContent()}
            </div>
          </div>
        </div>
